...
<div class="container centered footer">
    ...
    <?php
		...
		if ($prev != 0) {
			print '<li><a href="/?page='.$prev.'"><span class="glyphicon glyphicon-chevron-left" style="font-size: 70%"></span></a></li>'.PHP_EOL;
		}
		if ($page <= 5) {
			if ($numberOfPages <= 5) {
				for ($count = 1; $count <= $numberOfPages; $count++) {
					if ($page == $count) {
						print '<li><a class="active" href="/?page='.$count.'">'.$count.'</a></li>'.PHP_EOL;
					} else {
						print '<li><a href="/?page='.$count.'">'.$count.'</a></li>'.PHP_EOL;
					}
				}
			} else {
		...
		if ($next <= $numberOfPages) {
			print '<li><a href="/?page='.$next.'"><span class="glyphicon glyphicon-chevron-right" style="font-size: 70%"></a></li>'.PHP_EOL;
		}
	?>
    </ul>
</div>
